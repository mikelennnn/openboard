...
public final class InputLogic {
    ...
    private void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction,
        ...
        switch (event.getMKeyCode()) {
            ...
            case Constants.CODE_AI_ASSIST:
                handleAiAssistKey();
                break;
            case Constants.CODE_CORRECT_TEXT:
                handleCorrectTextKey();
                break;
            case Constants.CODE_RUDE_TEXT:
                handleRudeTextKey();
                break;
            case Constants.CODE_TRANSLATE_TEXT:
                handleTranslateKey();
                break;
            case Constants.CODE_FONT_STYLE:
                handleFontStyleKey();
                break;
            case Constants.CODE_SYMBOL_PANEL:
                mLatinIME.showSymbolPanel();
                break;
            case Constants.CODE_FORMAL_TEXT:
                handleFormalTextKey();
                break;
            case Constants.CODE_ROMANTIC_TEXT:
                handleRomanticTextKey();
                break;
            case Constants.CODE_DETAIL_TEXT:
                handleDetailTextKey();
                break;
            case Constants.CODE_SUMMARIZE_TEXT:
                handleSummarizeTextKey();
                break;
            case Constants.CODE_EMOJI_TEXT:
                handleEmojiTextKey();
                break;
            case Constants.CODE_SARCASM_TEXT:
                handleSarcasmTextKey();
                break;
     ...
     * Toma el texto que el usuario ya escribio antes del cursor y abre el panel
     * de chat con esa pregunta. Si no hay nada escrito, abre el panel vacio
     * (para ver el chat anterior) sin mandar nada nuevo.
     */
    private void handleAiAssistKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        final String question = TextUtils.isEmpty(textBeforeCursor)
                ? "" : textBeforeCursor.toString();
        mLatinIME.showAiPanel(question, question.length());
    }

    /**
     * Se llama cuando se presiona el boton "CORREGIR" de la barra del teclado.
     * Toma el texto ya escrito antes del cursor y le pide a la IA que corrija
     * ortografia y gramatica, sin abrir ningun panel: si funciona, reemplaza el
     * texto directamente por la version corregida. Si no hay texto, no hace nada.
     */
    private void handleCorrectTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Corrige la ortografia y gramatica del siguiente texto. "
                + "Responde unicamente con el texto ya corregido, sin explicaciones, "
                + "comentarios ni comillas adicionales:\n\n" + originalText;
        ...
        org.dslul.openboard.inputmethod.latin.OpenRouterAI.send(prompt,
            ...
            public void onError(final String errorMessage) {
                // Silencioso a proposito: si falla, se deja el texto original tal cual.
            }
        });
    }

    /**
     * Se llama cuando se presiona el boton de "reformular grosero" de la barra.
     * Toma el texto ya escrito antes del cursor y le pide a la IA que lo reformule
     * de forma grosera/enojada, sin abrir ningun panel; si funciona, reemplaza el
     * texto directamente. Si no hay texto, no hace nada.
     */
    private void handleRudeTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Reformula el siguiente texto para que suene grosero, "
                + "enojado y directo, como si la persona estuviera molesta, agregando "
                + "garabatos o insultos leves si encaja naturalmente, pero conservando "
                + "el mismo idioma y la misma idea principal del texto original. "
                + "Responde unicamente con el texto reformulado, sin explicaciones, "
                + "advertencias ni comillas adicionales:\n\n" + originalText;
    ...
    // Cuantos caracteres antes del cursor se toman como texto a traducir.
    private static final int TRANSLATE_MAX_CHARS = 4000;

    /**
     * Se llama al tocar el boton de traducir (a la izquierda de la barra). Toma el
     * texto ya escrito y abre el panel para elegir a que idioma traducirlo. Si no
     * hay texto escrito, no hace nada.
     */
    private void handleTranslateKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(TRANSLATE_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
            return;
        }
        final String text = textBeforeCursor.toString();
        mLatinIME.showTranslatePanel(text, text.length());
    }

    /**
     * Se llama al tocar el boton del lapiz (estilos de fuente). Toma el texto ya
     * escrito y abre el panel para elegir con que estilo Unicode reescribirlo.
     * Si no hay texto, no hace nada.
     */
    private void handleFontStyleKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(TRANSLATE_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String text = textBeforeCursor.toString();
        mLatinIME.showFontPanel(text, text.length());
    }

    /**
     * Se llama al tocar el boton del sombrero (formal/elegante). Toma el texto ya
     * escrito y le pide a la IA que lo vuelva mucho mas formal, elegante y
     * profesional, con ortografia perfecta; la IA puede agregar palabras o
     * extender el texto si hace falta. Reemplaza el texto en silencio, sin abrir
     * ningun panel. Si no hay texto, no hace nada.
     */
    private void handleFormalTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Reformula el siguiente texto para que suene formal, elegante "
                + "y profesional, con ortografia y gramatica perfectas. Puedes cambiar "
                + "palabras por otras mas elegantes y mejorar la redaccion, pero NO "
                + "agregues datos, ideas o informacion nueva que no estaba en el texto "
                + "original, y no lo hagas mas largo de lo necesario. Conserva el mismo "
                + "idioma, el mensaje y la intencion original. Responde unicamente con el "
                + "texto reformulado, sin explicaciones, comentarios ni comillas "
                + "adicionales:\n\n" + originalText;
    ...
    /**
     * Se llama al tocar el boton del corazon (romantico). Toma el texto ya escrito
     * y le pide a la IA que lo reformule de forma romantica/carinosa, ideal para
     * expresar afecto. Reemplaza el texto en silencio, sin abrir panel.
     */
    private void handleRomanticTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Reformula el siguiente texto para que suene romantico, "
                + "carinoso y afectuoso, ideal para expresarle amor o carino a alguien. "
                + "Conserva el mismo idioma y la idea principal del mensaje original. "
                + "Responde unicamente con el texto reformulado, sin explicaciones ni "
    ...
    /**
     * Se llama al tocar el boton de la hoja/documento (detallar). Toma el texto ya
     * escrito y le pide a la IA que lo explique con mas detalle, sin cambiar el
     * tono, para que se entienda mejor. Reemplaza el texto en silencio, sin panel.
     */
    private void handleDetailTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Reescribe el siguiente texto de forma un poco mas clara y "
                + "detallada SOLO para que se entienda mejor (por ejemplo, aclarando "
                + "expresiones ambiguas o completando ideas que quedaron incompletas). "
                + "NO agregues datos, hechos, ejemplos ni informacion que no estaba "
                + "implicita en el texto original, y se lo mas breve posible: agrega "
                + "solo lo minimo necesario para mayor claridad, nada mas. No cambies el "
                + "tono del texto original. Conserva el mismo idioma. Responde "
                + "unicamente con el texto resultante, sin explicaciones, comentarios ni "
                + "comillas adicionales:\n\n" + originalText;
    ...
    /**
     * Se llama al tocar el boton del rayo (resumir). Toma el texto ya escrito y le
     * pide a la IA que lo acorte a lo esencial. Reemplaza el texto en silencio.
     */
    private void handleSummarizeTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Resume el siguiente texto, dejando solo lo esencial, de la "
                + "forma mas breve posible sin perder el sentido principal. No agregues "
                + "informacion nueva. Conserva el mismo idioma. Responde unicamente con "
                + "el texto resumido, sin explicaciones ni comillas adicionales:\n\n"
                + originalText;
    ...
    /**
     * Se llama al tocar el boton de la carita (emojis). Toma el texto ya escrito y
     * le pide a la IA que le agregue algunos emojis relevantes, sin saturarlo.
     * Reemplaza el texto en silencio.
     */
    private void handleEmojiTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Agrega algunos emojis relevantes al siguiente texto, de "
                + "forma natural y sin saturarlo (pocos emojis, solo donde encajen bien, "
                + "nunca mas de uno por frase). No cambies las palabras del texto ni "
                + "agregues informacion nueva, solo inserta los emojis. Conserva el "
                + "mismo idioma. Responde unicamente con el texto resultante, sin "
    ...
    /**
     * Se llama al tocar el boton de la carita guinando (sarcasmo/chiste). Toma el
     * texto ya escrito y le pide a la IA que le agregue humor o sarcasmo.
     * Reemplaza el texto en silencio.
     */
    private void handleSarcasmTextKey() {
        final CharSequence textBeforeCursor =
                mConnection.getTextBeforeCursor(AI_ASSIST_MAX_CHARS, 0);
        if (TextUtils.isEmpty(textBeforeCursor)
                || TextUtils.getTrimmedLength(textBeforeCursor) == 0) {
        ...
        final String originalText = textBeforeCursor.toString();
        final int originalLength = originalText.length();
        final String prompt = "Reformula el siguiente texto para que suene gracioso o "
                + "sarcastico, con humor, manteniendo la misma idea principal. No agregues "
                + "datos nuevos que no estaban en el texto original. Conserva el mismo "
                + "idioma. Responde unicamente con el texto reformulado, sin "
                + "explicaciones ni comillas adicionales:\n\n" + originalText;
